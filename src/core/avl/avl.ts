...
export class AVLTree<K, V> {
	...
	searchNearestLte(k: K) {
		return this.tree.searchNearestLte(k);
	}
	searchNearestGte(k: K) {
	...
	betweenBounds(
		...
		return this.tree.betweenBounds(query, lbm as any, ubm as any);
	}
 ...
 * Node
 */
export class Node<K, V> extends BST<K, V> {
	...
	constructor(init: AVLOptions<K, V> = {}) {
		...
		this.data = init.hasOwnProperty("value") ? [init.value as V] : [];
	 ...
	 */
	rebalanceAlongPath(path: Node<K, V>[]) {
		let newRoot: Node<K, V> = this;
		let rotated;
		let i;
		...
		for (i = path.length - 1; i >= 0; i -= 1) {
			...
			path[i].height =
				1 +
				Math.max(
					t.left ? (t.left.height || 0) : 0,
					t.right ? (t.right.height || 0) : 0
				);
	...
	insert(key: K, value: V) {
		...
		while (true) {
			...
			if (currentNode.compareKeys(key, currentNode.key) < 0) {
				if (!currentNode.left) {
					insertPath.push(
						currentNode.createLeftChild({ key, value })
					);
					break;
				} else {
				...
				if (!currentNode.right) {
					insertPath.push(
						currentNode.createRightChild({ key, value })
					);
					break;
				} else {
